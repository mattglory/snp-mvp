;; Arkadiko Vault Strategy - TESTNET VERSION
;; Simplified for testnet deployment

;; Error codes
(define-constant ERR_UNAUTHORIZED (err u7200))
(define-constant ERR_INSUFFICIENT_BALANCE (err u7201))
(define-constant ERR_STRATEGY_INACTIVE (err u7202))
(define-constant ERR_INVALID_AMOUNT (err u7203))

;; Config
(define-constant MIN_DEPOSIT u1000000)
(define-constant SIMULATED_APY u550) ;; 5.5% APY
...
;; Read-only
(define-read-only (get-name) (ok "Arkadiko Vault (Testnet)"))
